// SERVER-ONLY. Calls the Anthropic API directly with fetch (no SDK dependency
// needed). ANTHROPIC_API_KEY must never be exposed to the browser — this file
// must only ever be imported from server code (route handlers / server actions).
...
async function callClaude(system: string, user: string): Promise<string> {
  ...
  if (!text.trim()) throw new Error("Claude API returned an empty response.");
  return text;
}
